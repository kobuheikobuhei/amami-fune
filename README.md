...
| 解析を直した後、既存記事を現在の判定に揃える | `node src/tools/resync-articles.js`（`--apply` で実行） |
...
毎日 日本時間22時30分に、公式の発表・台帳・ブログの3つを突き合わせる。
収集が動いていても、分類や対応づけが壊れていれば情報は静かに欠落する。
静かに欠落することが最も危険なため、毎日照合して食い違いを見つけたらメールで知らせる。
...
|---|---|
| 取得 | 監視先から取れていない |
| 分類 | 状態を判定できない発表がある（解析の失敗） |
| 抽出 | 日付を取り出せず継続中でもない発表がある（解析の失敗） |
| 矛盾 | 状態と表示の語が食い違っている |
| 欠落 | 公式に出ているのに台帳に無い |
| 未投稿 | 台帳にあるのに記事が作られていない |
| 重複 | 同じ内容の記事が複数ある |
| 消失 | 台帳が指す記事がブログに無い |
| 孤立 | 台帳に無い記事がブログにある |
| 陳腐化 | 継続中の案内が90日以上続いている |
| 配船 | 配船予定が古い、明日まで届いていない、両社の突き合わせで矛盾がある |

手元で実行するには `node src/audit.js`。
